...
use app\admin\service\platform\Category;
use app\common\models\Archives;
use app\common\models\goods\Goods;
use app\common\models\promotion\PromotionBonus;
use app\common\models\shop\Pages;
use app\common\models\shop\PagesItems;
...
class Shop extends Service  {
    ...
    public static function getLayoutData($id,$thumbImageDomain=false){
        if(!$row = Pages::where("id",$id)->find()){
        ...
        foreach($data as $key=>$value){
            ...
            if($value["widget_name"] == "article"){
                ...
                $arr = Category::getChildren($data[$key]['params']['article_id']);
                ...
                if ($data[$key]['params']['type'] == 'auto') {
                    if (isset($data[$key]['params']['classifyId']) && trim($data[$key]['params']['classifyId'])) {
                        $catIds = Category::getChildren($data[$key]['params']['classifyId']);
                    ...
                    $goodsData = Goods::field('id,cat_id,title,goods_number,goods_weight,sell_price,market_price,store_nums,photo,briefly,status,visit,favorite,sale')->where($where)->order('sale','desc')->limit($limit)->select()->toArray();
                    ...
                    if(!empty($arr)){
                        $res = array_map(function ($r) use($thumbImageDomain) {
                            ...
                            return $r;
                        },Goods::field('id,cat_id,title,goods_number,goods_weight,sell_price,market_price,store_nums,photo,briefly,status,visit,favorite,sale')->where([
                            "status"=>0
                ...
                $data[$key]['params']['list'] = PromotionBonus::where('status',0)->where("end_time",">",time())
                    ->where('type',0)
