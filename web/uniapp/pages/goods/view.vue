<template>
    <view>
		...
		<!-- #ifdef MP -->
		<navbar :scroll="scrollNum" :iSimmersive="false" :placeholder="true" title-color="#ffffff" background="#1b43c4" title="商品详情"></navbar>
		<!-- #endif -->
    ...
    methods: {
        ...
        loadGoodsData(){
            this.$http.getGoodsDetail({
				...
				this.url = this.$config.uni_app_web_url + "pages/goods/view?id=" + this.goods_id;
                ...
                this.$http.goodsDetailAddCart({
                    ...
                    if(result.status){
						...
						if(result.data.count > 0){
							uni.setTabBarBadge({ index: 3, text: result.data.count.toString() });
						}else{
