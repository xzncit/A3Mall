...
class Index extends Auth {
    ...
    public function detail(){
        ...
        $row["store"] = Db::name("store")->where("id",$row["store_id"])->find();
        if(!empty($row["store"])){
            $row["store"]["area_name"] = Area::get_area([$row["store"]["province"],$row["store"]["city"],$row["store"]["area"]],",");
        }
    ...
    public function refundment(){
        ...
        $row["goods"] = Db::name("order_goods")->where(["order_id" =>$row["id"]])->order("id DESC")->select()->toArray();
        $row["payment"] = Db::name("payment")->where("id",$row["pay_type"])->find();
    ...
    public function complete(){
        ...
        Db::name("order")->where(['id'=>$id])->update($orderData);
