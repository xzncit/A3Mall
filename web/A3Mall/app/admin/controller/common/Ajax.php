...
class Ajax extends Auth {
    ...
    public function get_users(){
        ...
        return View::fetch("common/get_users",[
    ...
    public function get_goods_list(){
        ...
        $cat = Db::name("category")->where(["status"=>0,"module"=>"goods"])->select()->toArray();
        return View::fetch("common/get_goods_list",[
            "cat"=>Data::analysisTree(Data::familyProcess($cat)),
            "brand"=>Db::name("products_brand")->where("status",0)->select()->toArray()
        ]);
    }
    ...
    public function get_goods_list_data(){
        $goods_id = Request::param("id", "0","trim,strip_tags");
        $result = Db::name("goods")->where("id","in",$goods_id)->select()->toArray();
        ...
        $html = View::fetch("common/get_goods_list_data",[
            "data"=>$result
        ]);

        return Response::returnArray("ok",1,[
            "content"=>$html,
            "goods_id"=>implode(",",$array)
        ]);
    }
