...
use mall\model\UsersSuggestModel;
...
class Suggest extends Auth {
    ...
    public function index(){
        if(Request::isAjax()){
            ...
            $usersConsult = new UsersSuggestModel();
    ...
    public function detail(){
        if(Request::isAjax()){
            ...
            Db::name("users_suggest")->where("id",$data["id"])->update($data);
            return Response::returnArray("操作成功");
        }

        $id = Request::param("id","0","intval");
        if(($row=Db::name("users_suggest")->where("id",$id)->find())==false){
    ...
    public function delete(){
        ...
        try {
            $row = Db::name("users_suggest")->where('id',$id)->find();
            if(empty($row)){
                throw new \Exception("您要查找的数据不存在！",0);
            }

            Db::name("users_suggest")->delete($id);
        } catch (\Exception $ex) {
