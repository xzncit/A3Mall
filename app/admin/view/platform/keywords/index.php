<div class="row content-nav">
    <div class="col-xs-12">
        <ul>
            ...
            <li><a href="javascript:;">词组列表</a></li>
        </ul>
    </div>
</div>
...
<script type="text/html" id="isTopcheckboxTpl">
    <input type="checkbox" name="is_top" value="{{d.id}}" title="开启" lay-filter="is-top-filter" {{ d.is_top == 0 ? 'checked' : '' }}>
</script>

<script type="text/html" id="isHotcheckboxTpl">
    <input type="checkbox" name="is_hot" value="{{d.id}}" title="开启" lay-filter="is-hot-filter" {{ d.is_hot == 0 ? 'checked' : '' }}>
</script>
    ...
    layui.use(['table','form'], function () {
        ...
        table.render({
            ...
            , cols: [[
                ...
                , {field: 'is_top', title: '置顶', width: 110,templet: '#isTopcheckboxTpl'}
                , {field: 'is_hot', title: '推荐', width: 110,templet: '#isHotcheckboxTpl'}
        ...
        //监听锁定操作
        form.on('checkbox(is-top-filter)', function(obj){
            var that = this;
            $.get('{:createUrl("field")}',{
                name : that.name,
        ...
        form.on('checkbox(is-hot-filter)', function(obj){
            var that = this;
            $.get('{:createUrl("field")}',{
                name : that.name,
                value : (obj.elem.checked ? 0 : 1),
