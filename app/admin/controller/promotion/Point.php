...
class Point extends Auth {
    ...
    public function editor(){
        if(!Request::isAjax()){
            ...
            $row = empty($id) ? [] : Db::name("promotion_point")->where("id",$id)->find();

            if(!empty($row)){
                $row = Db::name("promotion_point")->where(["id"=>$row["id"]])->find();
                if(empty($row)){
        ...
        $data = Request::post();
        $row = Db::name("promotion_point")->where(["goods_id"=>$data["goods_id"]])->find();
        if(!empty($row)){
            if($row["id"] != $data["id"]){
                return Response::returnArray("该商品己添加过积分兑换活动",0);
            }
        }
        ...
        if(empty($data["id"])){
            ...
            if(!Db::name("promotion_point")->insert($data)){
                return Response::returnArray("操作失败，请重试。",0);
            }
        }else{
            Db::name("promotion_point")->where(["id"=>$data["id"]])->update($data);
        }
