...
export default {
    ...
    methods: {
        onLoadOrder(){
            ...
            this.$request.get("/order/confirm",this.params).then((res)=>{
                ...
                if(res.status){
                    ...
                    if(order_address_id){
                        for(let obj in res.data.address.list){
                            if(res.data.address.list[obj].id == order_address_id){
                                ...
                                //this.$storage.delete("ORDER_ADDRESS_ID");
                                break;
                            }
                        }
                    ...
                    this.coupons = res.data.bonus.y;
                    this.disabledCoupons = res.data.bonus.n;
                }else{
                    Toast(res.info);
                }
            }).catch((err)=>{
        ...
        onChange(index) {
            ...
            if(index != -1){
                this.params.bonus_id = this.coupons[index].id;
            }else{
                this.params.bonus_id = 0;
            }

            this.bonusId = this.params.bonus_id;
            this.onLoadOrder();
        },
        onAdd() {
        ...
        selectAddress(item, index){
            ...
            this.chosenAddressId = this.addressList[index].id;
