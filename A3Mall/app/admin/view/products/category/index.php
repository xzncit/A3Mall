...
<script type="text/html" id="checkboxMenuTpl">
  <input type="checkbox" name="is_menu" value="{{d.id}}" title="开启" lay-filter="status-filter" {{ d.is_menu == 0 ? 'checked' : '' }}>
</script>

<script type="text/html" id="checkboxHotTpl">
  <input type="checkbox" name="is_hot" value="{{d.id}}" title="开启" lay-filter="status-filter" {{ d.is_hot == 0 ? 'checked' : '' }}>
</script>
...
layui.use(['table','form'], function () {
    ...
    table.render({
        ...
        , cols: [[
                ...
                , {field: 'is_menu', title: '菜单', width: 110,templet: '#checkboxMenuTpl',align:"center"}
                , {field: 'is_hot', title: '置顶', width: 110,templet: '#checkboxHotTpl',align:"center"}
