...
class Second extends Auth {
    ...
    public function delete(){
        ...
        try {
            $row = Db::name("promotion_second")->where('id',$id)->find();
            if(empty($row)){
                throw new \Exception("您要查找的数据不存在！",0);
            }

            Db::name("promotion_second")->delete($id);
    ...
    public function field(){
        ...
        try {
            Db::name("promotion_second")->where("id",$id)->update([$name=>$value]);
        } catch (\Exception $ex) {
            return Response::returnArray("操作失败，请稍候在试！",0);
        }
